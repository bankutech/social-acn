...
import { ArrowLeft, Image, Code, Type, Sparkles, X, Video, LayoutGrid as Grid } from 'lucide-react';
...
export default function CreatePostPage() {
  ...
  const handleFileSelect = (e) => {
    ...
    if (!file) return;
    if (createMode === 'post' || createMode === 'story') {
      setImageFile(file);
      setImagePreview(URL.createObjectURL(file));
      if (createMode === 'post') setPostType('image');
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (createMode === 'post') {
        ...
        if (imageFile) {
          const fd = new FormData(); fd.append('file', imageFile);
          const r = await api.upload('/api/upload/image/stories', fd);
          imageUrl = r.url; cloudinaryPublicId = r.filename;
        }
        await api.post('/api/stories', { content: content || 'My Story', type: imageFile ? 'image' : 'text', imageUrl, cloudinaryPublicId });
        navigate('/');
      } else if (createMode === 'reel') {
      ...
      <div className="page-inner">
        ...
        <header className="page-header">
          ...
          <div className="header-center">
            ...
            >
              {createMode === 'post' ? 'New post' : createMode === 'story' ? 'New story' : 'New reel'}
            </motion.span>
          </div>
              ...
              <div className="editor-card">
                ...
                {postType !== 'code' && (
                  ...
                  >
                    ...
                    {aiLoading ? 'Working…' : 'Polish with AI'}
                  </button>
                )}
            ...
            {createMode === 'story' && (
              <div className="story-wrap">
                <div className="story-canvas">
                  {imagePreview
                    ? <img src={imagePreview} alt="" className="story-bg" />
                    : (
                      <label className="story-upload">
                        <input type="file" accept="image/*" onChange={handleFileSelect} ref={fileInputRef} hidden />
                        <div className="upload-icon"><Image size={28} strokeWidth={1.5} /></div>
                        <strong>Pick a moment</strong>
                  ...
                  {imagePreview && (
                    <button className="remove-btn story-remove" onClick={removeFile}><X size={14} /></button>
                  )}
                </div>
              </div>
            )}
